...
class UserModelCase(unittest.TestCase):
    ...
    def test_generate_text(self):
        # Login as a user with a valid session
        self.client.post(
            "/auth/login",
            data={"email": "test@email.net", "password": "password"},
            follow_redirects=True,
        ...
        # Add additional assertions as needed for the specific behavior of the generate_text endpoint

        # Check that the generated_text is not empty
    ...
    def test_anonymous_user(self):
        ...
        with self.app.test_request_context():
            self.assertEqual(
                isinstance(current_user._get_current_object(), AnonymousUserMixin), True
            )
    ...
    def test_valid_registration(self):
        ...
        with self.client:
            ...
            response = self.client.post(
                "/auth/login",
                data={"email": "first@mail.com", "password": "Horse"},
                follow_redirects=True,
            )
            self.assertEqual(response.status_code, 200)
            # prompted to chat
